...
`Markdown` 没有官方统一的元数据表示方法。 `Pelican` 默认的方法如上图所示。

`Github` 支持在在开头的元数据以 `---` 开始和结尾，类似于
...
这么配置后，在支持的场景下将会把元数据渲染成表格的形式。但是在不支持的 `Markdown解析器` 上，将会把元数据渲染成一个大标题。
目前看，这么配置的元数据也能被 `Pelican` 正常解析。

为了在 `Github` （以及安装了 `Github Markdown Preview` 的 `VSCode` ）上获得更好的效果，我选择启用这个功能。
